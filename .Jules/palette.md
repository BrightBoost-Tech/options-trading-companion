## 2025-05-28 - Keyboard Navigation for Interactive Table Rows
**Learning:** Table rows that act as links or buttons (using `onClick` handlers) frequently lack keyboard accessibility and focus visibility. This makes them unreachable for users relying on keyboard navigation.
**Action:** When a `<TableRow>` or `<tr>` is interactive, ensure it has `tabIndex={0}`, an appropriate role (`role="link"` or `role="button"`), an `aria-label`, an `onKeyDown` handler (for 'Enter' and 'Space'), and clear `focus-visible` utility classes.
