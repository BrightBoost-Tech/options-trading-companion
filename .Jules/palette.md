...
## 2024-07-07 - WAI-ARIA Disclosure Pattern for Legacy View Toggle
**Learning:** Using standard `button` elements for expand/collapse toggles requires explicit implementation of the WAI-ARIA disclosure pattern (`aria-expanded` and `aria-controls` linked to a unique content `id`). Furthermore, when using Tailwind, `focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring` should be explicitly added to native buttons to ensure keyboard users have a clear visual indicator of focus, which is often missing or suppressed by default browser styles. In addition, when updating frontend components, Playwright E2E tests may fail if they rely on non-exact text matching (e.g. `getByText('Polygon')`) when the same substring might appear elsewhere or its conditional rendering logic has changed, highlighting the need for `{ exact: true }` in `not.toBeVisible()` assertions.
**Action:** Always verify that expand/collapse toggles implement `aria-expanded` and `aria-controls`, ensure native interactive elements have visible focus states (`focus-visible`), and update corresponding Playwright tests to use exact text matching when asserting element absence.
