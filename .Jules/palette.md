...
## 2025-06-08 - WAI-ARIA Disclosure Pattern
**Learning:** Collapsible sections and accordions built with native HTML `<button>` and `<div>` elements often lack necessary ARIA attributes to inform screen readers of their state. They also frequently miss explicit keyboard focus rings (`focus-visible`).
**Action:** When creating custom collapsible elements, explicitly include the WAI-ARIA disclosure pattern: `aria-expanded` on the toggle button and `aria-controls` pointing to a unique `id` generated by `React.useId()` on the content region. Also include `focus-visible` classes to ensure focus is always visible for keyboard users.
