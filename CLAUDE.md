...
3. **Empirical small-tier validation + credit spread emission verification (waiting on next small-tier cycle post-PR-\<this PR\>).** Question (d) from 2026-05-20 — "where did the 4 candidates die" — answered: PR #958 wiring gap (allocator output not consumed by sizing call). 2026-05-21 follow-on diagnostic answered "why only LONG_CALL_DEBIT_SPREAD emits" — chain-mechanics formula edge case (combo_spread / entry_cost on credit spreads). PR \<this PR\> closes both. Next small-tier cycle's empirical outcome determines: (i) does `created` go from 0 to ≥1, (ii) do credit spreads emit (90d empirical baseline: 0 emissions; expected to change post-merge), (iii) are the allocator's intended ~$186-$370 budgets actually flowing through `calculate_sizing` to produce `contracts_by_risk ≥ 1`, (iv) do `allocator_hint_dropped` and `chain_mechanics_formula_anomaly` stay at 0 alerts. Composition questions (a)-(c) — `limit=50` cap, `liquidity_score` staleness, score=50/60 ETF inclusion — remain deferred until 1-2 small-tier cycles with the wiring + formula fix complete produce real survival data. PR #960 hold-period audit is also unexercised because no debit-spread has closed since 2026-05-01; will become exercisable once trades start flowing post-PR-\<this PR\>.
