...
2. **H9 Convention Slot 1 — AST gate for wrappers-without-verification** — H9 Convention codified 2026-05-12 (see `docs/loud_error_doctrine.md` "Codified pattern with empirical anchors"). 3 infrastructure proposals ranked; Slot 1 is the recommended first ship (~half day, precedent PR #917 Class B AST gate, would have caught all 5 known H9 instances at PR time). Slot 2 (silent-exception grep test) queued behind; may consolidate with Slot 1 if AST gate covers same surface. Slot 3 (Literal status returns) adopted as convention for new wrappers — no infrastructure work, code review enforcement.
