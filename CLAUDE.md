...
3. **Universe composition review + small-tier candidate kill-site triage (data accumulating).** PR #970 (selection log) and PR \<this PR\> (early-exit symmetry) together complete the observability foundation for two composition/sizing questions. First cycle of small-tier data landed 2026-05-20 16:00 UTC — 4 candidates emitted, 0 created (hit `no_suggestions_after_gates`); cycle_metadata + selection log both validated. Open questions: (a) is `limit=50` the right policy cap given 69-symbol active universe, (b) should `liquidity_score` refresh frequency tighten (current snapshot 3+ weeks stale), (c) do score=50/60 ETFs belong on the active universe at all, (d) where in the small-tier path did today's 4 candidates die — allocator, sizing, H7, edge_below_minimum? Empirical observation continues for ~1 week before composition decisions. Doctrinal closure (H9 silent-decision generalization + early-exit symmetry) shipped with PRs #970 + \<this PR\>.
