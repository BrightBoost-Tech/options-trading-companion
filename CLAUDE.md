...
- **Universe:** 74 symbols active. PR #804 added F, BAC, SOFI, T, KO, VZ; PR #976 (2026-05-21) added KHC; PR \<data/universe-expansion-snap-rivn-nio-mara\> (2026-05-21 follow-on) added SNAP, RIVN, NIO, MARA; FISV deactivated 2026-05-19 (corporate action). Audit rows: KHC `2c91a730-be6c-4b7e-9433-630be9ddb1d2`, batch SNAP/RIVN/NIO/MARA `a23461cc-3661-45b7-96cb-e7a13dd0779f`.
...
- **Iron condors:** Enabled by pool construction. Triggered by `strategy_selector.get_candidates` when regime is CHOP, or sentiment is NEUTRAL/EARNINGS with high IV (iv_rank>50 or ELEVATED/SHOCK/REBOUND regime). Recent regimes have been NORMAL with directional sentiment → IC pool empty → debit spreads dominate. This is regime-driven natural selection. Empirical: 52 ICs emitted in 90d at regime=CHOP, 0 ICs at regime=NORMAL — by-design behavior. Phase-aware exclusion ALSO exists at `strategy_selector.py:372-387`: IRON_CONDOR is excluded when `CURRENT_PROGRESSION_PHASE=alpaca_paper`. Currently DORMANT in production (phase=micro_live since 2026-04-25). The 2026-04-30 "no phase-aware mechanism exists" verification was incorrect; corrected in PR \<this PR\> (2026-05-21).
...
| `universe_selection_log` | Per-call audit of `UniverseService.get_scan_candidates` (selected + dropped symbols, score thresholds, caller). Closes H9 silent-decision gap from 2026-05-19 funnel diagnostic. Forward-only as of 2026-05-20. |
...
**Early-exit symmetry (FIX 5, 2026-05-21):** `run_midday_cycle` previously emitted `cycle_metadata` only on the happy-path return at `workflow_orchestrator.py:3491`. The 7 early-exit returns (`micro_tier_position_open` 2042, `capital_scan_policy_block` 2058, `global_risk_budget_exhausted` 2121, `no_candidates` 2344, `scanner_failed` 2360, `no_suggestions_after_gates` 3197 + happy path 3491) now all emit cycle_metadata via the `_build_cycle_metadata` helper, with an explicit `exit_reason` field distinguishing the path. Pre-funnel exits encode "not measured" as `None`; post-funnel exits emit the values that were actually measured. v959 enriched counts (`scanner_emitted`, `trade_suggestions_created`, etc.) similarly emitted via `_build_enriched_counts` at every return path. Doctrine refinement: PR #959's "minimal counts shape for early-return paths" was correct for pre-scanner exits (2042/2058) and scanner-crashed exits (2360) but wrong for post-scanner exits (2344/3197) where the funnel ran and produced real measurements. Origin: 2026-05-20 verification report — first post-PR-#959 cycle to land on an early-exit return (the 16:00 UTC `no_suggestions_after_gates` path) surfaced the gap. See `docs/loud_error_doctrine.md` H9 "Early-exit observability symmetry" subsection.

**`universe_selection_log` (FIX 4, 2026-05-20):** every call to `UniverseService.get_scan_candidates` writes one row capturing the full selection decision — `total_active`, `limit_applied`, `selected_count`, `dropped_count`, `selected_symbols` + `dropped_symbols` (JSONB arrays), `score_threshold` (lowest score in selected set), `score_at_cutoff` (highest score in dropped set), and `metadata.caller`. Both production callers thread caller identity: `options_scanner.scan_for_opportunities` (limit=50) and `iv_daily_refresh.run` (limit=200). H9-verified writer: insert failure or empty PostgREST `data` response fires `universe_selection_log_write_failed` (severity=warning); the candidate-list return value is unaffected by observability-path failures. Closes the H9 silent-decision gap discovered in the 2026-05-19 funnel diagnostic — see `docs/loud_error_doctrine.md` H9 generalization subsection.
...
**Wiring completion (2026-05-21, PR \<this PR\>):** PR #958 shipped 2026-05-18 with the allocator + the `allocation_hint` parameter signatures on both consumer functions, but did not thread the hint from `workflow_orchestrator`'s sizing call site. Producer (allocator) wrote `_allocator_allocated_budget` to candidate dicts; no code read that field. The first small-tier cycle (2026-05-20 16:00 UTC: 4 candidates emitted, 0 created) was the empirical surface of the gap — sizing fell through to the legacy multiplier stack at ~$24.76 per-trade vs. the allocator's intended ~$186-$370. PR \<this PR\> completed the wiring at two call sites in `workflow_orchestrator.py` (sizing call passes `allocation_hint`; conditional bypass of `min(.., budgets.max_risk_per_trade)` clamp when `allocation_hint_applied=True` since the cycle-start RBE compute predates the allocator and would otherwise throttle the hint) and added a defensive `allocator_hint_dropped` alert at the consumer (`calculate_variable_sizing`) that fires when a future refactor drops the threading. PR #958 was correctly-designed-but-incompletely-wired; the doctrinal contribution (allocator + `allocation_hint` parameter mirror across both layers) holds. Doctrine entry: H9 verified-consumer generalization in `docs/loud_error_doctrine.md`.
...
- 2026-05-26: codification of EV-vs-H7 trade-off as the named structural constraint at small-tier capital (PR \<doctrine/ev-h7-tradeoff-codification\>) — documentation-only PR (no code, no tests) closing the structural arc of 2026-05-19 → 2026-05-26 (PRs #970/#972/#973/#974/#975/#976/#977/#978 plus three diagnostics this week). New section in `docs/structural_findings.md` capturing the binding constraint at $1,031 OBP: **high-priced underlyings produce sufficient EV but fail H7; sub-$30 underlyings pass H7 but produce insufficient EV after costs**. There is no candidate shape that simultaneously passes both at current capital — mechanical capital-vs-position-size arithmetic, not a code problem. Three operational paths (accept low frequency / capital scale / wait for regime change) named explicitly; six rejected code-side fixes catalogued with reasoning. H7_PREFILTER_ENABLED is ready for active-mode flip after multi-cycle shadow validation (2 cycles, 4 candidates, zero false positives) — operator-side Railway env change, not part of this PR. Reference for future sessions: structural_findings.md "EV-vs-H7 trade-off at small-tier capital".
- 2026-05-21: scanner_universe expansion — 4 additional sub-$30 underlyings (PR \<data/universe-expansion-snap-rivn-nio-mara\>) — added SNAP (~$6, social media, IV 0.62, 3.7% ATM bid-ask), RIVN (~$13.50, EV-US, IV 0.92, 3.3% ATM bid-ask), NIO (~$5.50, EV-China, IV 0.65, 11.4% ATM bid-ask), MARA (~$13.50, crypto mining, IV 0.87, 5.9% ATM bid-ask). Universe 70 → 74 active. Data-only change; no code touched. Follow-on to PR #976: KHC alone gave today's cycle 1 sub-$30 candidate (insufficient variance); these 4 additions diversify across 4 distinct sectors with IV-regime spread. Per H14 cite-then-verify doctrine, each candidate was verified against current Alpaca chain data (price, ATM bid-ask, IV/greeks populated, strike granularity, option volume). Rejected: HOOD (~$76 too high), PLTR (~$137 too high), PINS (~$18-19 but bid-ask 11-32% borderline), WBA (chain stale; corporate action), HBAN (deferred per PR #976 drift). All 4 missing α Phase 3 IV backfill — debit spreads emit with `iv_rank=50` default; IV-sensitive strategies (credit, IC) defer until backfill. Audit row `a23461cc-3661-45b7-96cb-e7a13dd0779f`.
- 2026-05-21: H7 allocator-aware pre-check + KHC composition addition (PR \<fix/h7-prefilter-and-khc-composition\>) — `workflow_orchestrator.run_midday_cycle` now filters candidates between `rank_and_select` and `PortfolioAllocator.allocate` against the same H7 math (`sizing_engine.estimate_close_bp`) the sizing engine uses downstream. Default `H7_PREFILTER_ENABLED=false` (shadow mode: log decisions, don't filter). Flip env to `true` for active mode after 1-2 cycles of shadow validation. New `cycle_metadata` fields `h7_prefilter_dropped` and `h7_prefilter_mode` (`shadow|active|disabled|error`); new `exit_reason='all_candidates_h7_unfit'` when active-mode filter drops everything. Closes `docs/small_tier_allocation.md` §7 item 5 (was deferred pending empirical evidence; 2026-05-21 cycles produced 4-of-4 H7 drops — the evidence). Composition: KHC added to scanner_universe (69 → 70 active, liquidity_score=75); HBAN deferred due to structural drift (live ~$38 vs `docs/structural_findings.md` $15.57 from 2026-05-14). Doctrine: new H14 "Reference-document freshness: cite-then-verify" entry — the fourth H9-family generalization this week (alongside silent-decision PR #970, early-exit symmetry PR #972, verified-consumer PR #973). 33 new tests in `test_h7_prefilter.py` + 2 pre-existing PR #972 tests updated to recognize the new exit_reason value. Audit row `2c91a730-be6c-4b7e-9433-630be9ddb1d2`. Companion PR #975 (credit-spread chain-mechanics fix) merged independently.
- 2026-05-21: credit spread chain-mechanics formula fix (PR #975) — `options_scanner.py` chain-mechanics gate now uses `combo_spread / max_loss_share` for credit spreads (denominator switch from entry_cost). Empirical surface: 0 credit spreads in `trade_suggestions` over 90 days; today's worker logs showed every `SHORT_*_CREDIT_SPREAD` attempt hitting the `spread=200.0%` artifact from the legacy `combo_spread / entry_cost` formula on small credit values. Debit spread / single-leg long paths unchanged. Plus defensive `chain_mechanics_formula_anomaly` alert (severity=warning) fires when `spread_pct > SPREAD_PCT_ANOMALY_THRESHOLD` (default 300%) — H9 verified-consumer doctrine applied to gate behavior so future formula regressions surface within one cycle rather than 90 days. Doctrinal correction: α Phase 3's "unlocks IV-sensitive strategies" framing precisely means data-side decidability, NOT emission. Credit spread emission was blocked by chain-mechanics, not by α. Iron condor emission remains regime-gated by design (CHOP / NEUTRAL+high-IV / EARNINGS+high-IV). Also fixes CLAUDE.md's incorrect "no phase-aware mechanism exists in code" verification claim — the mechanism at `strategy_selector.py:372-387` exists but is dormant under production phase=micro_live. 17 new tests in `test_credit_spread_emission.py`.
- 2026-05-21: small-tier allocator wiring completed (PR \<this PR\>) — `workflow_orchestrator.py` sizing call now threads `allocation_hint=cand.get("_allocator_allocated_budget")` to `calculate_variable_sizing`; the cycle-start RBE clamp is conditionally bypassed when `allocation_hint_applied=True` so the allocator's per-candidate budget isn't throttled back to the legacy ~3% stack. Defensive `allocator_hint_dropped` alert added at the consumer (severity=high) — fires if a future refactor drops the threading, instead of producing another silent-cycle outage. Origin: 2026-05-20 small-tier kill-site investigation surfaced PR #958's incomplete wiring after the first small-tier cycle produced 4 candidates and 0 created. PR #958's design holds; this PR is the wiring completion. Doctrine: H9 verified-consumer generalization added in `docs/loud_error_doctrine.md`.
- 2026-05-21: cycle_metadata + enriched_counts now emitted at all 7 return paths of `workflow_orchestrator.run_midday_cycle` (PR \<this PR\>) via `_build_cycle_metadata` / `_build_enriched_counts` helpers. `exit_reason` field distinguishes the path; pre-funnel exits emit `None` for unmeasured fields, post-funnel exits emit measurements. Closes the asymmetric-observability gap surfaced by the 2026-05-20 verification (16:00 UTC cycle hit the `no_suggestions_after_gates` early-exit — first post-PR-#959 cycle to land on any early-exit path — and the cycle_metadata field was NULL). Discovery during code-read: PR #959's investigation reported 5 early-exit returns; the actual count is 6 (the 7th return is the happy path). The missed return is `global_risk_budget_exhausted` at line 2121; the fix covers all 6 + happy path. Doctrine entry: "Early-exit observability symmetry" added under H9 silent-decision generalization in `docs/loud_error_doctrine.md`. `ops_health_service._resolve_regime_for_staleness` consumer updated to iterate past pre-budget exits (regime=None) to find a cycle with regime populated rather than fail-closed to 'shock' on the first None.
- 2026-05-20: H9 silent-decision generalization (PR #970) — `universe_service.get_scan_candidates` now writes `universe_selection_log` rows capturing both selected AND dropped symbols with score thresholds. Closes the observability gap discovered in the 2026-05-19 funnel diagnostic (19 of 34 never-emitter symbols were silently dropped by the `limit=50` truncation with zero queryable trace). Doctrine generalization: H9 extended from "verified-write" (error-handling shape) to "verified-decision" (selection-boundary shape). Ride-along: FISV deactivated from `scanner_universe` (Fiserv renamed to FI in 2024 corporate action). Migration `20260519000001_universe_selection_log.sql`.
...
**Working backlog: see `docs/backlog.md` (post-diagnostic, grouped + sequenced — Groups 1-7 + highest-leverage items).**
...
3. **Operator-side decision on small-tier path forward (codified 2026-05-26).** The structural arc through `canonical_ranker` closed this week (PRs #970/#972/#973/#974/#975/#976/#977/#978 plus codification in `docs/structural_findings.md` "EV-vs-H7 trade-off at small-tier capital"). Pipeline is structurally healthy through the EV gate. The binding constraint at $1,031 OBP is mechanical (high-priced underlyings fail H7; sub-$30 underlyings produce insufficient EV after costs) and the canonical_ranker correctly enforces it. **Three operational paths are operator-side decisions**, not code workstreams: (a) accept low frequency at small tier; (b) scale capital to standard tier ($5k+, where the same gross EVs become economically viable through size); (c) wait for market regime change (CHOP regime historically produced 69 ICs at avg EV $70; last CHOP window was 2026-02-24 to 2026-03-17). Six code-side levers have been catalogued and rejected as ineffective at small tier — see `docs/structural_findings.md` "What does NOT resolve this constraint". **Sub-action ready when operator decides:** `H7_PREFILTER_ENABLED=true` env flip on Railway worker service — multi-cycle shadow validation across 2026-05-22 and 2026-05-26 confirmed pre-check decisions match real-H7 outcomes (4 candidates flagged, zero false positives). Flag flip changes `exit_reason` from `no_suggestions_after_gates` to `all_candidates_h7_unfit` for cleaner observability; doesn't change trade-execution outcome. PR #908 empirical validation (focus #1) still waiting on next natural close — orthogonal to this focus.
...
**α IV pipeline:** full-universe historical `iv_rank` decidable for 67 of 70 active universe symbols (Phase 3 complete 2026-05-17). 2 symbols (WBD, XLK) at the 60-row threshold; 1 sparse (BKNG) at 30 rows — `daily_refresh` closes the BKNG gap naturally by mid-July 2026. α Phase 3 scope was DATA-side: it backfilled IV30 rows making `iv_rank` computable. Emission of IV-sensitive strategies (credit spreads, iron condors) is gated SEPARATELY: iron condors are regime-gated (CHOP / NEUTRAL+high-IV / EARNINGS+high-IV — by-design); credit spreads were blocked for 90+ days by a chain-mechanics formula edge case at `options_scanner.py:~3149` (fixed 2026-05-21 in PR \<this PR\>). The "α unlocked IV-sensitive strategies" framing in earlier notes meant data decidability, not emission. Full history at `docs/alpha_iv_history.md`.
...
- **Universe:** 70 symbols (post-Phase-3 α coverage; 67 at full iv_rank decidability; KHC added 2026-05-21 via PR \<fix/h7-prefilter-and-khc-composition\>)
