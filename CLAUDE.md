...
3. **Empirical small-tier validation post-H7-pre-check + KHC composition (waiting on next small-tier cycle).** PR \<fix/h7-prefilter-and-khc-composition\> shipped both the H7 pre-check (shadow mode by default) AND KHC composition addition. Companion PR #975 (credit-spread chain-mechanics fix) merged independently. Next small-tier cycle's observable outputs determine: (i) does KHC reach the allocator (`universe_selection_log` includes `KHC` in `selected_symbols`), (ii) does KHC emit a debit spread candidate (`emission_counts_by_strategy` shows non-zero for sub-$30 underlying), (iii) what's the H7 shadow-drop count (`cycle_metadata.h7_prefilter_dropped`; expected high — today's 4-of-4 pattern), (iv) do credit spreads emit (90d empirical baseline: 0 emissions; PR #975 should unblock), (v) does any `trade_suggestion` get created. If shadow drops trace cleanly to today's 4-of-4 pattern, flip `H7_PREFILTER_ENABLED=true` for active mode. PR #960 hold-period audit + the `allocator_hint_dropped` / `chain_mechanics_formula_anomaly` defensive alerts continue to monitor. HBAN deferred pending refreshed structural assessment (live ~$38 vs docs $15.57). Composition questions (a)-(c) — `limit=50` cap, `liquidity_score` staleness, score=50/60 ETF inclusion — remain deferred.
