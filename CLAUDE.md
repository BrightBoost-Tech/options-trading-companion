...
3. **Universe composition review (waiting on 1-2 cycles of `universe_selection_log` data).** This PR's silent-decision closure (\<this PR\>) is observability-only — the `limit=50` cap, `liquidity_score` staleness (3+ weeks per 2026-05-19 query), and the bottom-tier ETF clustering at score=50/60 are all composition questions the operator wants deferred until the new selection log produces real cycle data. Expected timeline: ~1 week of cycles. Open questions: (a) is `limit=50` the right policy cap given 70-symbol universe, (b) should `liquidity_score` refresh frequency tighten, (c) do score=50/60 ETFs (DIA, SMH, XLC, XL-series) belong on the active universe at all. Composition decisions wait for data; this slot tracks the wait. Doctrinal closure (H9 silent-decision generalization) shipped with the observability PR.
