...
3. **Empirical small-tier validation (waiting on next small-tier cycle post-PR-\<this PR\>).** Question (d) from 2026-05-20 — "where did the 4 candidates die" — answered: PR #958 wiring gap (allocator output not consumed by sizing call). PR \<this PR\> completes the wiring. Next small-tier cycle's empirical outcome determines: (i) does `created` go from 0 to ≥1 (or does another gate downstream of sizing — H7, edge_below_minimum — bind), (ii) are the allocator's intended ~$186-$370 budgets actually flowing through `calculate_sizing` to produce `contracts_by_risk ≥ 1`, (iii) does `allocator_hint_dropped` stay at 0 alerts. Composition questions (a)-(c) — `limit=50` cap, `liquidity_score` staleness, score=50/60 ETF inclusion — remain deferred until 1-2 small-tier cycles with the wiring complete produce real survival data. PR #960 hold-period audit is also unexercised because no debit-spread has closed since 2026-05-01; will become exercisable once trades start flowing post-wiring-fix.
