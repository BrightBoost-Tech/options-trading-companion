...
const getScoreAndColor = (ev: number, maxLoss: number) => {
  if (maxLoss <= 0) { // Positive EV on risk-free trade is great
    return { score: 95, color: 'bg-green-500', label: 'Excellent', progressColor: '[&>div]:bg-green-500' };
  }

  const evToRiskRatio = (ev / maxLoss) * 100;

  if (evToRiskRatio >= 25) {
    return { score: 90, color: 'bg-green-500', label: 'Excellent', progressColor: '[&>div]:bg-green-500' };
  } else if (evToRiskRatio >= 15) {
    return { score: 75, color: 'bg-yellow-500', label: 'Good', progressColor: '[&>div]:bg-yellow-500' };
  } else if (evToRiskRatio >= 5) {
    return { score: 60, color: 'bg-yellow-400', label: 'Fair', progressColor: '[&>div]:bg-yellow-400' };
  } else {
    return { score: 40, color: 'bg-red-500', label: 'Poor', progressColor: '[&>div]:bg-red-500' };
  }
};
...
export const TradeScoreCard = ({
  ...
  const { score, color, label, progressColor } = getScoreAndColor(expected_value, max_loss);
  ...
  return (
    <div className="bg-white rounded-lg shadow-md border border-gray-200 p-4 w-full">
      ...
      <div className="mb-4">
          <Progress
            value={score}
            className={`w-full bg-gray-200 h-2.5 ${progressColor}`}
