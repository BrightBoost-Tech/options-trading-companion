...
function HoldingsTreemap({ exposure, loading }: HoldingsTreemapProps) {
  ...
  const strategies = useMemo(() => {
    ...
    return Object.entries(exposure.byStrategy).filter(([, v]: any) => v > 1); // Only show > 1% strategies
  }, [exposure?.byStrategy, loading]);

  if (loading)
    return <div className="h-48 bg-muted animate-pulse rounded"></div>;
        ...
        <div className="space-y-3">
          {sectors.length === 0 ? (
            <p className="text-sm text-muted-foreground italic">
              No significant sector exposure.
            </p>
          ) : (
