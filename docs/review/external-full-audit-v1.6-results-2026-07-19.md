...
| premium-basis shadow | `risk_basis_shadow` | `RBE:418` / `util_gate:353` | logs `[RISK_BASIS_SHADOW]` | `choose_basis` (flag OFF) | tests | **NEVER EMITTED** (ledger) | **DEGRADED-INSTRUMENT** | arm gate cannot clear |
...
**Instrument headline:** the only non-GOOD/non-CLEAN durable instruments are (a) the **nightly completion contract** (PARTIAL/HIGH — the F-RUNNER-WORKTREE-DEADFALLBACK root cause), (b) the **premium-basis `[RISK_BASIS_SHADOW]`** emitter (DEGRADED-INSTRUMENT — never emitted, blocks the P0-B arm gate), and (c) **HMAC signed-task** (PARTIAL — behavioral suite skipped). Everything else is GOOD/CLEAN or truthfully HONEST-EMPTY/DARK.
...
- **Retained (A4 · two).**
  - **F-A4-RISKBASIS-SILENT (MED).** Sizing/utilization stay on premium basis and the `[RISK_BASIS_SHADOW]` evidence that gates arming `RISK_BASIS_MAX_LOSS_ENABLED` **has never emitted** (`risk_basis_shadow.py:31` ← `risk_budget_engine.py:418`, `utilization_gate.py:353`; ledger.md:1030/1236) → the observe→enforce gate can never clear. VERIFIED-CODE + VERIFIED-DB(ledger). P0-B + A9 marker-silence.
...
| 2 | **F-A4-RISKBASIS-SILENT** | A4 | **MED** | VERIFIED-CODE + VERIFIED-DB(ledger) | The `[RISK_BASIS_SHADOW]` comparison instrument has never emitted (`risk_basis_shadow.py:31`); its silence blocks the P0-B arm gate so sizing/utilization stay on premium basis indefinitely. NEW. |
...
| F-A4-RISKBASIS-SILENT | MED | VERIFIED-CODE + VERIFIED-DB(ledger) | RETAINED | P0-B book-scaling epic | EXTENDS | P0-B/P2 | P0-B (backlog.md:1365) + A9 marker-silence | new | Decide whether to emit `[RISK_BASIS_SHADOW]` in a natural cycle so the ~1-week arm review can begin | A natural midday cycle log shows `[RISK_BASIS_SHADOW]` with `would_flip` populated |
...
2. **Decide the P0-B arm path:** the `[RISK_BASIS_SHADOW]` instrument must emit in a natural cycle before the ~1-week observe→enforce arm review can even begin; today it is silent, so the gate can never clear.
...
- **Strongest evidence-quality gap:** the multi-basis reconciliation's empty natural cross-basis overlap (TCM v2 0/528, ranker `ranking_costs` 0/86, dispositions/provenance 0) — plus the never-emitted `[RISK_BASIS_SHADOW]` instrument (F-A4-RISKBASIS-SILENT).
