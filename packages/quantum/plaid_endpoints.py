...
def parse_plaid_error(e: ApiException) -> str:
    ...
    try:
        ...
        # 🛡️ Sentinel: Fallback to generic error if JSON parse fails to prevent leaking sensitive body
        return "Plaid Provider Error (Invalid Response)"


def _is_dev_env() -> bool:
    """Helper to check if current environment is development or test"""
    return os.getenv("APP_ENV", "development") in ["development", "test"]
    ...
    async def get_plaid_status(
        ...
        try:
            ...
            res = (
                ...
                .select("plaid_access_token, plaid_institution")
                .eq("user_id", user_id)
                .single()
                .execute()
            )
            ...
            if res.data and res.data.get("plaid_access_token"):
                return {
                    "connected": True,
                    "institution": res.data.get("plaid_institution")
                    or "Connected Broker",
                }
        ...
        try:
            ...
            if analytics_service:
                analytics_service.log_event(
                    user_id, "plaid_link_error", "system", {"error": str(e)}
                )

            # 🛡️ Sentinel: Mask specific error details in production
            detail = str(e) if _is_dev_env() else "Configuration Error"
            ...
            if analytics_service:
                analytics_service.log_event(
                    user_id, "plaid_link_error", "system", {"error": error_msg}
                )
            ...
            raise HTTPException(
                status_code=500,
                detail="Internal Server Error: Failed to create link token.",
            )
        ...
        try:
            ...
            if analytics_service:
                analytics_service.log_event(
                    ...
                    "ux",
                    {"institution": metadata.get("institution", {}).get("name")},
                )
            ...
            raise HTTPException(
                status_code=500, detail="Internal Server Error: Token exchange failed."
            )
        ...
        try:
            ...
            if holdings_list:
                ...
                try:
                    supabase.table("positions").upsert(
                        positions_to_insert, on_conflict="user_id,symbol"
                    ).execute()
                    print("✅ Positions updated in DB")
                except Exception as e:
                    print(f"❌ Failed to update positions in DB: {e}")
                    raise HTTPException(
                        status_code=500, detail="Database update failed."
                    )
            ...
            if accounts_list:
                ...
                try:
                    ...
                    print(
                        "✅ Portfolio Snapshot updated with buying_power and holdings"
                    )
            ...
            raise HTTPException(
                status_code=500,
                detail="Internal Server Error: Failed to retrieve holdings.",
            )
