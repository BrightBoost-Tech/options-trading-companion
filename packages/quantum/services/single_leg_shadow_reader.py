    ...
    sections = {
        "epoch": _query_section(
            ...
            .select(
                ...
                "live_submit_allowed,config_hash,version,enabled_at,"
                "enabled_by,created_at"
            )
            .eq("epoch_name", epoch),
        ),
        "policies": _query_section(
            ...
            .select(
                ...
                "approval_status,effective_epoch,config_hash,policy_config,"
        ...
        "bindings": _query_section(
            ...
            .select(
                ...
                "routing_mode,execution_mode,enabled,created_at"
            )
            .eq("epoch_name", epoch)
            .eq("user_id", user_id),
        ),
        "runs": _query_section(
            ...
            .select(
                ...
                "policy_epoch,policy_registration_id,portfolio_id,user_id,as_of,"
                "status,counts,error_details,created_at,started_at,finished_at"
            )
        ...
        "attempts": _query_section(
            ...
            .select(
                ...
                "direction,strategy_type,stage,reason_code,"
                "candidate_fingerprint,occ_symbol,strike,expiry,"
                "debit_per_contract,ev_expected_value,ev_pop,ev_basis,ev_model,"
                "considered_contracts,viable_contracts,provider,known_at,created_at"
            )
            .eq("user_id", user_id),
        ),
        "events": _query_section(
            ...
            .select(
                "event_id,run_id,policy_registration_id,user_id,event_type,"
                "entity_type,entity_id,candidate_fingerprint,payload,"
        ...
        "orders": _query_section(
            ...
            .select(
                ...
                "candidate_fingerprint,symbol,occ_symbol,option_type,"
                "strategy_type,contracts,fill_price_per_share,debit_total,"
                "source_known_at,routing_mode,execution_mode,lifecycle_state,"
                "live_submit_allowed,status,filled_at,created_at"
            )
            .eq("user_id", user_id),
        ),
        "positions": _query_section(
            ...
            .select(
                "position_id,order_id,run_id,policy_registration_id,portfolio_id,"
                "user_id,candidate_fingerprint,symbol,occ_symbol,option_type,"
                "strategy_type,strike,expiry,contracts,entry_price_per_share,"
                "entry_debit_total,status,routing_mode,execution_mode,"
                "lifecycle_state,live_submit_allowed,opened_at,closed_at,"
                "terminal_spot,terminal_value,realized_pnl,close_reason"
            )
            .eq("user_id", user_id),
        ),
        "outcomes": _query_section(
            ...
            .select(
                "outcome_id,position_id,run_id,policy_registration_id,"
                "portfolio_id,user_id,candidate_fingerprint,symbol,strategy_type,"
                "opened_at,closed_at,entry_debit_total,terminal_value,"
                "realized_pnl,close_reason,execution_mode,experiment,is_paper,"
                "created_at"
            )
            .eq("user_id", user_id),
        ),
        "cash_events": _query_section(
            ...
            .select(
                "cash_event_id,portfolio_id,policy_registration_id,user_id,"
                "order_id,position_id,event_type,amount,balance_before,"
...
def summarize_single_leg_shadow_sections(
    ...
    attempts_by_stage = Counter(
        str(row.get("stage") or "<null>") for row in attempts
    )
    rejections = Counter(
        (
        ...
        for row in attempts
        if row.get("stage")
        in ("selection_rejected", "gate_rejected", "execution_rejected")
    )
    events_by_type = Counter(
        str(row.get("event_type") or "<null>") for row in events
    )
    runs_by_status = Counter(
        str(row.get("status") or "<null>") for row in runs
    )
    positions_by_status = Counter(
        str(row.get("status") or "<null>") for row in positions
    )
    ...
    for policy_id in policy_ids:
        policy_attempts = [
            row
            for row in attempts
            if row.get("policy_registration_id") == policy_id
        ]
        policy_orders = [
            row for row in orders if row.get("policy_registration_id") == policy_id
        ]
        policy_positions = [
            row
        ...
        policy_runs = [
            row for row in runs if row.get("policy_registration_id") == policy_id
        ]
        policy_summary[policy_id] = {
            ...
            "generated_candidates": sum(
                ...
                if row.get("stage") == "candidate_generated"
            ),
            "internal_fills": len(policy_orders),
            "open_positions": sum(
                1 for row in policy_positions if row.get("status") == "open"
            ),
            "closed_positions": sum(
                1 for row in policy_positions if row.get("status") == "closed"
            ),
            "outcomes": len(policy_outcomes),
            "realized_pnl": round(
                sum(_number(row.get("realized_pnl")) for row in policy_outcomes),
                2,
            ),
        }
    ...
    return {
        ...
        "epoch": {
            ...
            "live_submit_allowed": (
                epoch.get("live_submit_allowed") if epoch else None
            ),
        ...
        "policy_counts": {
            "total": len(policies),
            "approved": sum(
                1 for row in policies if row.get("approval_status") == "approved"
            ),
            "draft": sum(
                1 for row in policies if row.get("approval_status") == "draft"
            ),
        ...
        "binding_counts": {
            ...
            "experimental": sum(
                1 for row in bindings if row.get("role") == "experimental"
            ),
        },
        "headline": {
            "runs": len(runs),
            "attempts": len(attempts),
            "generated_candidates": attempts_by_stage.get(
            ...
            "realized_pnl": round(
                sum(_number(row.get("realized_pnl")) for row in outcomes), 2
            ),
        ...
        "policies": {
            key: policy_summary[key] for key in sorted(policy_summary)
        },
        "isolation": {
            ...
            "execution_modes": sorted(
                {
                    ...
                    for row in orders + outcomes
                    if row.get("execution_mode")
                }
            ),
            "live_submit_true_rows": sum(
                1
                for row in orders + positions
                if row.get("live_submit_allowed") is True
            ),
            "non_one_contract_orders": sum(
                1 for row in orders if row.get("contracts") != 1
            ),
        },
    }
