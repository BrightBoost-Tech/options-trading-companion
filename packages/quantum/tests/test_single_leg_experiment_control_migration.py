...
EXPECTED = {
    "sl_exp_throughput_v1": (
        "71e854a6e9f098d561748b49161c5997459b4f2a7a19e27eebcb741c1987db5e"
    ),
    "sl_ctrl_throughput_v1": (
        "441ace2f5dc5b7842f6ae41db30db3dcd32ffbb1afa5585794659b04421fb310"
    ),
    "sl_exp_conviction_v1": (
        "59e02e8f09b3030f7fa5f3cd6f281ee42e80100e73f2a6e8fdcfe1e56374cf09"
    ),
    "sl_ctrl_conviction_v1": (
        "5f74bffe2d819d850f9c74be992b82f353a0ff15d5d2912abd9fb96502fc7de0"
    ),
}
...
def test_setup_is_disabled_single_user_and_two_portfolio_only():
    sql = _sql().lower()
    setup = sql[
    ...
    assert "state <> 'disabled'" in setup
    assert "'shadow_only'" in setup
    assert "'internal_paper'" in setup
    assert "live_submit_allowed" in setup
    assert "single-leg experiment v1 already belongs to another user" in setup
    assert "sl_exp_throughput_v1" in setup
    assert "sl_exp_conviction_v1" in setup
    binding_tail = setup.split("insert into single_leg_experiment_bindings")[-1]
    assert "sl_ctrl_throughput_v1" not in binding_tail
    assert "on conflict (epoch_name) do nothing" in setup
    assert "expected 2 disabled bindings" in setup
    assert "enabled_bindings', v_enabled" in setup
...
def test_approval_never_enables_and_enable_requires_exact_approved_set():
    ...
    assert "another user has enabled single-leg bindings" in enable
...
def test_pause_is_global_persisted_kill_switch_without_deleting_history():
    sql = _sql().lower()
    pause = sql[sql.index("function rpc_pause_single_leg_experiment_v1") :]
    assert "set enabled = false" in pause
    assert "where epoch_name = 'single_leg_experiment_v1'" in pause
...
def test_control_migration_itself_writes_no_business_rows_or_flags():
    sql = _sql().lower()
    prefix = sql[
        : sql.index("create or replace function rpc_setup_single_leg_experiment_v1")
    ]
