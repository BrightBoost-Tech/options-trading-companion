...
from scripts.analytics.single_leg_shadow_report import render_markdown
...
def test_reader_keeps_empty_and_failed_sections_distinct():
    sections = fetch_single_leg_shadow_sections(Client(fail={"single_leg_shadow_cash_events"}), USER)
...
def test_reader_preserves_policy_control_and_isolation_axes():
    ...
    assert summary["policies"]["sl_exp_throughput_v1"]["matched_control_family"] == (
        "aggressive"
    )
...
def test_markdown_is_deterministic_and_contains_no_user_identifier():
    sections = fetch_single_leg_shadow_sections(Client(), USER)
    payload = {
