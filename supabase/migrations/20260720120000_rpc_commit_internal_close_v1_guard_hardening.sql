-- =============================================================================
-- V17-1.7  rpc_commit_internal_close_v1 — two follow-up guard hardenings
-- =============================================================================
-- NOT APPLIED BY THIS PR (draft). ADDITIVE `CREATE OR REPLACE FUNCTION` of the
-- ALREADY-APPLIED rpc_commit_internal_close_v1 (migration
-- 20260719180000_rpc_commit_internal_close_v1.sql). This migration replaces the
-- function BODY ONLY — it re-creates NO tables, adds NO columns, builds NO
-- index, and rewrites NO business rows. The 14-arg signature, the write-once
-- marker / idempotency contract, the server-side cash derivation, the fixed
-- search_path, and the operator-only grant surface are all byte-preserved.
-- Applied only via the operator-owned migration procedure
-- (docs/migration_procedure.md).
--
-- The two follow-ups (Lane-1B review of the applied v1):
--
--  FOLLOW-UP 1 — ACCEPT-GATE CLARITY (routing_mode). The applied v1 already
--   rejects a non-shadow book with the fail-safe ALLOWLIST
--   `routing_mode IS DISTINCT FROM 'shadow_only'` (accepts ONLY 'shadow_only';
--   rejects 'live_eligible', 'alpaca_live', NULL, and ANY future third routing
--   mode such as the inert #1003 'paper_shadow'). That allowlist is the SAFEST
--   predicate and is KEPT as the authoritative gate. This follow-up adds an
--   explicit, more-specific `routing_mode = 'live_eligible'` branch ABOVE it so
--   the broker book is handled DELIBERATELY (its own clear error) rather than
--   incidentally — a CLARITY/robustness tweak with ZERO behavior delta: every
--   input maps to the identical accept/reject outcome as v1 (live_eligible was,
--   and remains, rejected).
--
--  FOLLOW-UP 2 — NON-FINITE PROVENANCE GUARD (p_fill_mid_reference). v1 guards
--   its four cash-math numerics against NaN/±Infinity but NOT the provenance-
--   only p_fill_mid_reference. This is a REAL data-integrity fix, not cosmetic:
--   on PostgreSQL 17 `jsonb_build_object(k, x::numeric)` serializes a non-finite
--   x to the JSON STRING "NaN"/"Infinity"/"-Infinity" WITHOUT raising (verified
--   read-only on the production instance, PG 17.6, 2026-07-20), so an unguarded
--   non-finite p_fill_mid_reference is DURABLY STORED in BOTH paper_orders.
--   order_json AND the paper_ledger fill metadata, poisoning the #1017 fill-
--   quality provenance learning reads to weight/exclude mid-fallback fills (H9).
--   The guard is added to the existing FIX-2 non-finite check and is NULL-safe:
--   NULL (the designed default) `IN (...)` evaluates to NULL, never TRUE, so a
--   null/absent provenance still passes; a finite reference still passes; ONLY
--   NaN/±Infinity reject with the same typed `non_finite_input` error.
-- =============================================================================

-- =============================================================================
-- rpc_commit_internal_close_v1  (CREATE OR REPLACE — body only)
-- =============================================================================
...
BEGIN
    ...
    IF p_fill_price_magnitude IN ('NaN'::numeric, 'Infinity'::numeric, '-Infinity'::numeric)
       ...
       -- (v1.7 FOLLOW-UP 2) provenance-only param, but STILL guard it: on
       -- PostgreSQL 17 jsonb_build_object serializes a non-finite numeric to the
       -- JSON STRING "NaN"/"Infinity"/"-Infinity" WITHOUT raising, so an
       -- unguarded non-finite p_fill_mid_reference is durably STORED in BOTH
       -- order_json and the ledger metadata, poisoning the #1017 fill-quality
       -- provenance learning reads. NULL is designed-valid (the default):
       -- NULL IN (...) evaluates to NULL, never TRUE, so a null/absent
       -- provenance still passes — ONLY NaN/±Infinity reject; a finite
       -- reference is accepted unchanged.
       OR p_fill_mid_reference IN ('NaN'::numeric, 'Infinity'::numeric, '-Infinity'::numeric)
    THEN
    ...
    -- (b) a non-'shadow_only' portfolio is the broker book — this DB guard
    -- mirrors the route's own P0-A gate (execution_router.should_submit_to_broker
    -- is True iff routing_mode='live_eligible'; the internal-fill block runs only
    ...
    END IF;
    -- (v1.7 FOLLOW-UP 1) Handle the live book DELIBERATELY with its own specific
    -- error, THEN fall through to the fail-safe allowlist. The allowlist
    -- immediately below is AUTHORITATIVE and the SAFEST predicate: it accepts
    -- ONLY 'shadow_only' and rejects 'live_eligible', 'alpaca_live', NULL, AND
    -- any future third routing mode (e.g. the inert #1003 'paper_shadow') by
    -- construction — an internal committer must never touch a book it does not
    -- recognize. This explicit branch is a CLARITY/robustness tweak ONLY: it
    -- does NOT change the accept/reject outcome for ANY input.
    IF v_portfolio.routing_mode = 'live_eligible' THEN
        RAISE EXCEPTION
            'commit_internal_close: live_order_forbidden (routing_mode=live_eligible)';
    END IF;
...
COMMENT ON FUNCTION rpc_commit_internal_close_v1(
    ...
    'position is a typed reject. Operator-only (service_role). v1.7 hardening: '
    'explicit live_eligible reject (clarity, zero behavior delta) + non-finite '
    'guard on the provenance-only p_fill_mid_reference (a non-finite value would '
    'otherwise be stored as a JSON string). Foundation for V17-1 F-A2; the '
    'Python route is switched to call it in Lane 1B.';

-- =============================================================================
-- Operator-only execution surface (byte-identical to the applied v1 grants;
-- re-issued so this migration is self-contained. CREATE OR REPLACE preserves
-- the existing ACL, so these are idempotent.)
-- =============================================================================
