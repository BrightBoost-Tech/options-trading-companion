...
BEGIN
    SELECT string_agg(
               b.policy_registration_id || ':' || pr.config_hash || ':' || b.portfolio_id::text,
...
BEGIN
    ...
    -- hash. Controls must omit the opt-in; experiment rows must carry it.
    SELECT count(*) INTO v_bad
     ...
     WHERE pr.policy_registration_id IS NULL
        ...
        OR (
            e.experimental
            AND lower(coalesce(pr.policy_config->>'single_leg_experiment_enabled','false'))
    ...
    IF v_bad <> 0 THEN
        RAISE EXCEPTION 'single-leg manifest registry contract failed for % row(s)', v_bad
            USING ERRCODE = '23001';
    END IF;
    ...
    SELECT count(*) INTO v_bad
     ...
     WHERE epoch_name = 'single_leg_experiment_v1'
       AND (
           ...
           OR version <> 1
           OR state NOT IN ('disabled','paused','enabled')
       );
    IF v_bad <> 0 THEN
        RAISE EXCEPTION 'single-leg epoch contract drifted' USING ERRCODE = '23001';
    END IF;
    ...
    LOOP
        ...
        IF FOUND THEN
            ...
            IF NOT FOUND
               ...
               OR v_binding.execution_mode <> 'internal_paper' THEN
                RAISE EXCEPTION 'existing experimental binding/portfolio drifted for %',
    ...
    SELECT count(*) INTO v_bad
     ...
     WHERE epoch_name = 'single_leg_experiment_v1'
       AND user_id = p_user_id;
    IF v_bad <> 2 THEN
        RAISE EXCEPTION 'single-leg setup expected 2 bindings, got %', v_bad
            USING ERRCODE = '23001';
    END IF;
...
BEGIN
    ...
    IF v_draft <> 4 OR v_approved <> 0 THEN
        RAISE EXCEPTION 'policy approval state is mixed or incomplete'
            USING ERRCODE = '23001';
    END IF;
    ...
    UPDATE policy_registrations
       SET approval_status = 'approved',
           approved_at = now(),
           created_by = coalesce(created_by, p_approved_by)
     WHERE effective_epoch = 'single_leg_experiment_v1'
       AND policy_registration_id IN (
           SELECT policy_registration_id
             FROM single_leg_experiment_expected_policies_v1()
...
REVOKE ALL ON FUNCTION single_leg_experiment_expected_policies_v1() FROM PUBLIC, anon, authenticated;
REVOKE ALL ON FUNCTION single_leg_experiment_current_fingerprint_v1(uuid,numeric) FROM PUBLIC, anon, authenticated;
REVOKE ALL ON FUNCTION rpc_setup_single_leg_experiment_v1(uuid,numeric,text) FROM PUBLIC, anon, authenticated;
REVOKE ALL ON FUNCTION rpc_approve_single_leg_experiment_v1(uuid,text,text) FROM PUBLIC, anon, authenticated;
REVOKE ALL ON FUNCTION rpc_enable_single_leg_experiment_v1(uuid,text,text) FROM PUBLIC, anon, authenticated;
REVOKE ALL ON FUNCTION rpc_pause_single_leg_experiment_v1(uuid,text) FROM PUBLIC, anon, authenticated;

GRANT EXECUTE ON FUNCTION single_leg_experiment_expected_policies_v1() TO service_role;
GRANT EXECUTE ON FUNCTION single_leg_experiment_current_fingerprint_v1(uuid,numeric) TO service_role;
GRANT EXECUTE ON FUNCTION rpc_setup_single_leg_experiment_v1(uuid,numeric,text) TO service_role;
GRANT EXECUTE ON FUNCTION rpc_approve_single_leg_experiment_v1(uuid,text,text) TO service_role;
GRANT EXECUTE ON FUNCTION rpc_enable_single_leg_experiment_v1(uuid,text,text) TO service_role;
GRANT EXECUTE ON FUNCTION rpc_pause_single_leg_experiment_v1(uuid,text) TO service_role;

COMMENT ON FUNCTION rpc_setup_single_leg_experiment_v1 IS
    'Creates/validates disabled single-leg epoch plus exactly two shadow-only internal-paper experimental portfolios/bindings; never approves or enables.';
...
COMMENT ON FUNCTION rpc_pause_single_leg_experiment_v1 IS
    'Immediate persisted kill switch: disables experimental bindings and pauses new child scans while preserving evidence/open positions.';
